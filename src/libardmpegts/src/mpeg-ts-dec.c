...
int ts_demuxer_flush(void* _ts)
{
	struct ts_demuxer_t *ts = _ts;
    uint32_t i, j;
...
int ts_demuxer_input(void* _ts, const uint8_t* data, size_t bytes)
{
	struct ts_demuxer_t *ts = _ts;
    int r = 0;
    uint32_t i, j, k;
	...
	if(0x02 & pkhd.adaptation_field_control)
	{
		...
		if(pkhd.adaptation.adaptation_field_length > 0 && pkhd.adaptation.PCR_flag)
		{
		    ...
		    printf("pcr: %02d:%02d:%02d.%03d - %" PRId64 "/%u\n",
			      ...
			      pkhd.adaptation.program_clock_reference_base,
			...
			for(j = 0; j < ts->pat.pmt_count; j++)
			{
				...
				else
				{
					for (k = 0; k < ts->pat.pmts[j].stream_count; k++)
					{
                        ...
                        {
                            uint32_t n;
                            n = (uint32_t)pes_read_header(pes, data + i, bytes - i);
                            assert(n > 0);
                            i += n;
...
void* ts_demuxer_create(ts_demuxer_onpacket onpacket, void* param)
{
...
int ts_demuxer_destroy(void* _ts)
{
	struct ts_demuxer_t *ts = _ts;
    size_t i, j;
    struct pes_t* pes;
...
int ts_demuxer_getservice(void *_ts, int program, char* provider, int nprovider, char* name, int nname)
{
    struct pmt_t* pmt;
	struct ts_demuxer_t *ts = _ts;
...
void ts_demuxer_set_notify(void *_ts, struct ts_demuxer_notify_t* notify, void* param)
{
	struct ts_demuxer_t *ts = _ts;
